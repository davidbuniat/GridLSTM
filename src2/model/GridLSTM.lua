...
--[[
  ...
  --]]
local GridLSTM = {}
function GridLSTM.grid_lstm(input_size, output_size, rnn_size, n, dropout, should_tie_weights)
  ...
  if should_tie_weights == 1 then shared_weights = {nn.Linear(rnn_size, 4 * rnn_size),nn.Linear(rnn_size, 4 * rnn_size), nn.Linear(rnn_size, 4 * rnn_size)} end
  ...
  for L = 1,n do
    ...
    if L == 1 then
      ...
      prev_h_d = nn.Linear(input_size, rnn_size)(inputs[2]):annotate{name='input'} -- input_h_d: the starting depth dimension hidden state. We map a char into hidden space via a lookup table
    else 
    ...
    local x2h_t = nn.Linear(rnn_size, 4 * rnn_size)(prev_h_x):annotate{name='x2h_'..L}
    local y2h_t = nn.Linear(rnn_size, 4 * rnn_size)(prev_h_y):annotate{name='y2h_'..L}
    ...
    local next_c_x, next_h_x = lstm(x2h_t, y2h_t, d2h_t, prev_c_x, rnn_size)
    local next_c_y, next_h_y = lstm(x2h_t, y2h_t, d2h_t, prev_c_y, rnn_size)
    ...
    -- Evaluate the input sums at once for efficiency
    local x2h_d = nn.Linear(rnn_size, 4 * rnn_size)(next_h_x):annotate{name='i2h_'..L}:annotate{name='eval_1'}
    local y2h_d = nn.Linear(rnn_size, 4 * rnn_size)(next_h_y):annotate{name='i2h_'..L}:annotate{name='eval_2'}
    local d2h_d = nn.Linear(rnn_size, 4 * rnn_size)(prev_h_d):annotate{name='h2h_'..L}:annotate{name='eval_2'}
    ...
    local next_c_d, next_h_d = lstm(x2h_d, y2h_d, d2h_d, prev_c_d, rnn_size)
