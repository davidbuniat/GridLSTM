...
def saveImage(Y, name):
    Y = Y
    new_y = np.empty((row_count*column_count, 3))
    for i in range(len(new_y)):
        #print(int(round(Y[i][0]*155)) )
        new_y[i] = labels_rev[int(round(Y[i][0]*157)) ]
...
def main(num_epochs=NUM_EPOCHS):
    ...
    try:
        ...
        saveImage(y_test[0], "y_GT.png")
        saveImage(get_prediction(X_test)[0], "y_output.png")
