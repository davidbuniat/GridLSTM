...
local data = {}
data.__index = data
function data.create(n_sequence, width, height, nbatches, batch_size) 
	----------------------------------------------------------------------
	...
	function self:next_batch()
		...
		for i = 1, batch_size do 
			...
			batch_x[i] = trainData.data[self.current_batch*(self.batch_size-1)+i]
			batch_y[i] = trainData.labels[self.current_batch*(self.batch_size-1)+i]
		end
