...
local data = require 'data'             -- LSTM timestep and utilities
...
cmd:text()
cmd:text('Training a simple LSTM')
cmd:text()
cmd:text('Options')
...
cmd:option('-seq_length',19200,'number of timesteps to unroll to')
...
local loader = data.create(n_sequence, width,height, nbatches, batch_size)
local label_size = loader.labels_size  -- the number of distinct characters

-- define model prototypes for ONE timestep, then clone them
--
local protos = {}

protos.embed = nn.Linear(3, opt.rnn_size)
-- lstm timestep's input: {x, prev_c, prev_h}, output: {next_c, next_h}
protos.lstm = LSTM.lstm(opt.rnn_size)
protos.softmax = nn.Sequential():add(nn.Linear(opt.rnn_size, label_size)):add(nn.LogSoftMax())
